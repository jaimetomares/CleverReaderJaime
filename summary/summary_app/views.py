...
import os
import nltk     #set of libraries and programs for Python that allow us to carry out many tasks related to Natural Language Processing.
import pytesseract   #OCR tool for python. It can read and recognize text in images and is commonly used in python ocr image to text use cases.
import re
import slate3k as slate  #Python package that simplifies the process of extracting text from PDF files. It depends on the PDFMiner package.
import pdf2image
...
def index(request):
    if request.method == 'POST':
        ...
        pdfFileObj = open(os.path.join(settings.MEDIA_ROOT, filename), "rb")
